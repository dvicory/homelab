...
use homelab_preserve::{plan, points, restore, status, validate_document, verify};
...
fn route_result<'a>(report: &'a Value, route_id: &str) -> &'a Value {
    report["routes"]
        .as_array()
        .and_then(|routes| routes.iter().find(|route| route["routeId"] == route_id))
        .expect("route result is present")
}
...
fn point_representations_are_checked_against_the_integration() {
    ...
    let report = points(&document, "state").unwrap();
    assert_eq!(
        route_result(&report, "route")["error"]["code"],
    ...
    let report = points(&document, "state").unwrap();
    assert_eq!(
        route_result(&report, "route")["points"]
            .as_array()
            .unwrap()
            .len(),
        1
...
fn point_consistency_and_fidelity_must_satisfy_the_route() {
    ...
    let report = points(&satisfied, "state").unwrap();
    assert_eq!(
        route_result(&report, "route")["points"]
            .as_array()
            .unwrap()
    ...
    let report = points(&weak, "state").unwrap();
    assert_eq!(
        route_result(&report, "route")["error"]["code"],
    ...
    let report = points(&unknown, "state").unwrap();
    assert_eq!(
        route_result(&report, "route")["error"]["code"],
        "unsupported-point-consistency"
    );
    ...
    let report = points(&missing_fidelity, "state").unwrap();
    assert_eq!(
        route_result(&report, "route")["error"]["code"],
    ...
    let report = points(&incomplete, "state").unwrap();
    assert_eq!(
        route_result(&report, "route")["error"]["code"],
        "incomplete-point"
    );
...
fn independent_routes_keep_separate_point_evidence() {
    ...
    let report = points(&document, "state").unwrap();
    let first_point = &route_result(&report, "route")["points"][0];
    let second_point = &route_result(&report, "route-stale")["points"][0];
    assert_eq!(first_point["captureId"], "capture-a");
    assert_eq!(second_point["captureId"], "capture-b");
    assert_eq!(first_point["capturedAt"], "2026-01-01T00:00:00Z");
    assert_eq!(second_point["capturedAt"], "2026-02-02T00:00:00Z");

    fs::remove_dir_all(directory).unwrap();
}

#[test]
fn points_report_keeps_failed_routes_separate() {
    let directory = temporary("points-report");
    let describe = describe_result(FULL_CAPABILITIES);
    let good_points = format!(
        "{{\"protocolVersion\":1,\"requestId\":\"%s\",\"result\":{{\"kind\":\"points\",\"points\":[{}]}}}}",
        point_json("route", "capture-a", "filesystem", "[\"posix-filesystem\"]")
    );
    let ok = op_adapter(
        &directory,
        &[("describe", &describe), ("points", &good_points)],
    );

    let assert_failed = |second: PathBuf, code: &str| {
        let document = two_route_document(ok.clone(), second, "resolved");
        let report = points(&document, "state").unwrap();
        let first = route_result(&report, "route");
        assert_eq!(first["status"], "ok");
        assert_eq!(first["points"].as_array().unwrap().len(), 1);
        assert_eq!(first["points"][0]["captureId"], "capture-a");
        let failed = route_result(&report, "route-stale");
        assert_eq!(failed["status"], "failed");
        assert_eq!(failed["points"].as_array().unwrap().len(), 0);
        assert_eq!(failed["error"]["code"], code);
        assert_eq!(failed["error"]["stateId"], "state");
        assert_eq!(failed["error"]["routeId"], "route-stale");
        document
    };

    let exit_directory = directory.join("exit");
    fs::create_dir(&exit_directory).unwrap();
    let exiting = script(
        &exit_directory,
        &format!(
            "request=$(cat)\nid=$(printf '%s' \"$request\" | sed -n 's/.*\"requestId\":\"\\([^\"]*\\)\".*/\\1/p')\ncase \"$request\" in *'\"operation\":\"points\"'*) exit 7;; esac\nprintf '{describe}' \"$id\"\n"
        ),
    );
    let mut exited_document = assert_failed(exiting, "adapter-exit");

    let scratch_root = directory.join("scratch");
    fs::create_dir(&scratch_root).unwrap();
    exited_document.scratch_destinations.insert(
        "scratch".to_owned(),
        ScratchDestination {
            native_parent: "fixture/scratch".to_owned(),
            mount_root: scratch_root,
        },
    );
    assert_eq!(
        restore(
    ...
    let malformed = op_adapter(
        &malformed_directory,
        &[("describe", &describe), ("points", "{")],
    );
    assert_failed(malformed, "malformed-adapter-response");

    let weak_directory = directory.join("weak");
    fs::create_dir(&weak_directory).unwrap();
    let weak_points = format!(
        "{{\"protocolVersion\":1,\"requestId\":\"%s\",\"result\":{{\"kind\":\"points\",\"points\":[{}]}}}}",
        point_json("route-stale", "capture-b", "live", "[\"posix-filesystem\"]")
    );
    let weak = op_adapter(
        &weak_directory,
        &[("describe", &describe), ("points", &weak_points)],
    );
    assert_failed(weak, "unsupported-point-consistency");
