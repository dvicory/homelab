...
func run(argv []string) error {
	...
	server, err := incus.ConnectIncusUnixWithContext(context.Background(), incusSocket, &incus.ConnectionArgs{
	...
	if args.operation == "create" && instance != nil {
		if oldReference != "" && selected != oldReference {
...
func checkPreseed(server incus.InstanceServer, spec descriptor) ([]string, error) {
	...
	if missingProject {
		...
		for key, expected := range spec.ProjectConfig {
			if project.Config[key] != expected {
...
func checkIDMap(server incus.InstanceServer, spec descriptor, instance *api.Instance) error {
	...
	end := spec.IDMapBase + spec.IDMapSize
	if instance != nil {
	...
	for _, allocationPath := range []string{"/etc/subuid", "/etc/subgid"} {
		...
		for _, allocation := range allocations {
			if allocation.owner == "root" && allocation.base <= spec.IDMapBase && spec.IDMapBase <= end && allocation.base+allocation.count >= end {
				covered = true
				break
			}
		}
		if !covered {
			return fmt.Errorf("root lacks the declared subordinate range in %s", allocationPath)
		}
		if strings.HasSuffix(allocationPath, "subgid") {
			for _, row := range spec.IDMap["gid"] {
				if row.NSID != row.HostID || row.Range != 1 {
					continue
				}
				covered = false
				for _, allocation := range allocations {
					if allocation.owner == "root" && allocation.base <= row.HostID && row.HostID < allocation.base+allocation.count {
						covered = true
						break
					}
				}
				if !covered {
					return fmt.Errorf("root lacks subordinate coverage for capability GID %d in %s", row.HostID, allocationPath)
				}
			}
		}
		for _, allocation := range allocations {
			if allocation.owner != "root" && allocation.base < end && allocation.base+allocation.count > spec.IDMapBase {
				return fmt.Errorf("declared ID range overlaps another owner in %s", allocationPath)
			}
		}
	}
	...
	for _, other := range allInstances {
		...
		if !present {
			...
			return fmt.Errorf("inspect effective ID map for %s/%s: empty ID map", other.Project, other.Name)
		}
		...
		for _, row := range rows {
			if row.hostid < end && row.hostid+row.rangeN > spec.IDMapBase {
				return fmt.Errorf("ID range overlaps %s/%s", other.Project, other.Name)
			}
		}
	}
	return nil
