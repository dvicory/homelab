...
func SecretNamespaces(ctx context.Context, tools *Tools, input []byte) ([]string, error) {
	output, err := tools.RunYQInput(ctx, input, "-r", "-N", ".metadata.namespace | select(. != null)", "-")
	if err != nil {
		return nil, errors.New("unable to list secret namespaces from the staged manifest")
	}
