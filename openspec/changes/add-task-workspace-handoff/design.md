...
- `task_run_activations`: broker-issued activation ID, task, Kanban run, workspace, lease, policy digest, monotonic epoch, and state (`active`, `consumed`, `superseded`).
...
3. The broker binds the finalization ID to source authority and request digest, consumes the task-run activation, closes the VM, publishes and verifies the revision, then returns its opaque ID and digest.
