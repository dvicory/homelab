Desired-state repointing and fail-closed media-loss behavior are complete.
Creation-tier policy, backing-location declarations, placement/capacity checks,
and link-aware movement remain open; production data has not been migrated.
...
- [x] 3.1 Point the writable media boundary of the acquisition workloads at the namespace, keeping the common parent so that import can link, and replacing the current per-application library roots with the namespace layout. Verified by `media-contracts`, which confirms no consumer-declared volume references the compute state root.
- [x] 3.2 Give the player read-only access to the library subtree only. Verified by `media-contracts` and the x86_64 replacement acceptance.
...
- [x] 3.4 Regenerate `docs/operations.md` and confirm its retained-state table no longer lists a media mapping.
