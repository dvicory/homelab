...
- [x] 1.1 Record the disposable-control-plane proposal, storage delta, design, and ADR update, then verify `openspec validate --change disposable-control-plane-recovery` passes.
- [x] 1.2 Revise overlapping `reliable-household-services` recovery/backup tasks so they no longer require the deleted packaged restore/resume path, then verify `openspec validate --change reliable-household-services` passes.

## 2. Remove obsolete recovery implementation

- [x] 2.1 Delete `household-recovery`, its inventory/script, recovery freshness alerts, capture-check plumbing, offline Jellyfin fixtures, and sandbox artifacts, then verify no remaining references except historical decision records.
