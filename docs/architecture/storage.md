...
The originating review is to be recorded in ADR-0006 (stable semantic storage
namespace), which is pending and not yet in this repository.
...
- `/srv/media/data` is declared as one mergerfs namespace over the existing
  ...
  LUKS2/XFS target.
- Media4 remains declared `provisioned = false` with XFS as its pending
...
boundary.
The decision and its alternatives are to be recorded in ADR-0007 (block-level
device encryption), which is pending and not yet in this repository.
...
**Separate physical realization.** Media4 is the reserved, empty
LUKS2/XFS destination. Its read-only preflight measures the direct media1
source, target capacity, stable target identity, and target emptiness before
any formatter approval. Revision A realizes and mounts media4 while the
mergerfs branches stay media1–media3. After the complete direct media1 tree is
copied and verified on media4 outside mergerfs, separately approved Revision B
changes the running branch set from media1 to media4 atomically. Media1 is
never an evacuation source or formatter target in this operation; it remains
mounted and unchanged as the rollback copy until a later, separately approved
media1 conversion. No physical conversion has been executed.
...
When a declared branch set changes, the mergerfs service restarts during the
NixOS activation because its rendered mount script is a restart trigger. Do not
treat a branch-set change as a live reload or promise a later restart gate:
activation is the cutover, after all intended providers are mounted and
writers are quiesced. Consumers whose bind mounts need to see the remounted
namespace follow the measured cutover procedure.
...
- ADR-0006 (pending, not yet in this repository) — the decision behind this
  direction and the options rejected.
- ADR-0007 (pending, not yet in this repository) — the decision to encrypt data
  devices with block-level containers instead of a userspace layer.
- [LUKS2/XFS storage migration](../operations/luks-storage-migration.md) —
  generated procedure and declared-disk inventory for the conversion tooling.
