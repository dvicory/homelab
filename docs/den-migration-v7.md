# Den Migration v7 — Implementation-Aware

> **Post-Phase-4 paths.** Incorporates Phase 3/4 addendums and v6 adversarial review.
> **Key v7 changes:** Phase 1 design updated to actual implementation. Phase 3 deferred (fork-gated). Phase 4 slash notation documented with root cause. Per-host user config for remote-unlock noted.

---

# (Original v6 content follows)
...
**Note:** Phase 0.1's `settings.core.nix.gc.enable` was declared separately before the full `settings` submodule was added. In practice, the individual option declaration at `den.schema.host.options.settings.core.nix.gc.enable` targets the `freeformType` of the `settings` submodule directly — freeform key injection, not NixOS module option merging. Phase 2's `options.settings = lib.mkOption { ... }` wraps the same `freeformType` in a typed submodule; the individual keys remain compatible through the freeform space.
...
## Phase 3: Quirks
> **DEFERRED — fork-gated.** Quirk pipes and collectors require the den fork. Current implementation keeps `persist.directories` NixOS option. See [Phase 3 Addendum](./den-migration-v6-phase3-addendum.md).
...
## Phase 4: Rename ## Phase 4: Rename & Reorganize Reorganize
> **Completed.** Multi-word dot-notation aspect names cause `hasAspect` failures — `lazyAttrsOf` processes nested keys as content wrappers without `name`/`meta`. Slash notation (`den.aspects."disk/zfs"`) is the working approach. See [Phase 4 Addendum](./den-migration-v6-phase4-addendum.md).
