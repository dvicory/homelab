...
- **Managed `kanban_create` cannot create rough `triage` intake.** The internal database API accepts `create_task(..., triage=True)`, but the managed model-facing schema intentionally omits and rejects `triage`; operational escalation can still move repeatedly blocked existing tasks into that status. Do not design a conversational rough-task flow around a nonexistent managed entry point. Keep unapproved decomposition proposals in conversation state, and reserve `triage` for operator attention unless a separate durable proposal lifecycle is implemented.
...
- **Helper files under `modules/` must be `_`-prefixed.** `import-tree` treats every `.nix` file as a flake-parts module in one namespace; plain function libraries (e.g. `modules/den/aspects/secrets/_generators.nix`, `modules/packages/_install-on-envoy.nix`) are skipped only when underscore-prefixed. Import them from the consuming module with `import ./_file.nix`.
...
- **Do not strip the `|| return 1` guards in hermes-deployctl, and add them to new critical commands.** Its deploy functions are invoked under `if !`, where bash silently disables `errexit` — without the guards a failed activation continues to the canary against the old workload.

- **The full Hermes overlay `tests/` tree is not the verification surface.** Running every test in the patched source locally takes >30 minutes and includes ~9 pre-existing environment-dependent failures (`tests/plugins/video_gen/test_fal_plugin.py::TestFamilyRouting`, `test_kanban_attachments.py::test_worker_context_lists_attachments_with_absolute_path`) that also fail against the pristine pinned source. `check.nix` runs a curated pytest list — add new test files there explicitly (both the pytest invocation and the ruff list) or they are never exercised by `nix build .#checks.aarch64-darwin.hermes-worker-lane`.
