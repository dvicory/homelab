...
- Four 1 TB Intel NVMe devices are available and two are installed. Each installed controller reports 1,000,204,886,016 bytes total, one approximately 100.9 GB namespace, and approximately 899.3 GB unallocated capacity. The boot `rpool` occupies one small namespace and is 84% allocated with 70% fragmentation. Safe root expansion/reinstallation and allocation of a separate owned-device NVMe mirror must be designed before durable application state is placed.
- Seven 12 TB HDDs are attached. Three temporary single-device Btrfs/gocryptfs filesystems provide 36.00 TB raw, 21.04 TB used, and 14.96 TB estimated free through mergerfs. `bulk-2` and `bulk-3` are imported read-write as online single-disk ZFS pools and reference 7.03 TB and 6.81 TB in their `medialibrary` datasets. They may be repurposed only after their unique files are reconciled. Two more disks have no recognized filesystem. No SnapRAID parity exists today.
