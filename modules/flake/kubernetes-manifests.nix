{ inputs, lib, self, rootPath, ... }:
{
  perSystem =
    ...
    let
      ...
      generatedManifests = if builtins.pathExists manifestSource then manifestSource else throw ''
    ...
    in
    {
      ...
      checks.prod-home-manifests-fresh = pkgs.runCommandLocal "prod-home-manifests-fresh" {
      ...
      checks.prod-home-gitops-source = pkgs.runCommandLocal "prod-home-gitops-source" {
        src = generatedManifests;
        nativeBuildInputs = [
        ...
        covered = {"apps"} | {path.name for path in root.iterdir() if path.is_dir() and path.name != "apps"}
