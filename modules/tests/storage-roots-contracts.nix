...
let
  ...
  isCanonicalPath = import ../den/aspects/services/_storage-path.nix { inherit lib; };
  ...
  assertions = {
    "canonical absolute paths are accepted" = builtins.all isCanonicalPath [
      "/srv/media"
      "/mnt/storage-clear/media1"
    ];
    "ambiguous roots are rejected" = builtins.all (path: !isCanonicalPath path) [
      "/"
      "srv/media"
      "/srv//media"
      "/srv/media/"
