        ...
        let
          ...
          test = pkgs.testers.runNixOSTest {
                  ...
                  services.nginx = {
                    ...
                    virtualHosts.origin = {
                      ...
                      locations."/headers".return =
                        "200 $remote_addr|$http_x_forwarded_for|$http_x_forwarded_host|$http_forwarded|$http_x_forwarded_user|$http_x_auth_request_user";
                    };
                  };
                };
              remote =
            ...
            testScript = ''
              ...
              client.succeed("curl --cacert ${certificates}/edge.crt -sSf --resolve jellyfin.primary.test:443:10.0.0.11 https://jellyfin.primary.test/ | grep -qx origin")
              client.succeed("curl --cacert ${certificates}/edge.crt -sSf --resolve jellyfin.backup.test:443:10.0.0.12 https://jellyfin.backup.test/ | grep -qx origin")
              client.succeed("curl --cacert ${certificates}/edge.crt -sI --resolve primary.test:443:10.0.0.11 https://primary.test/ | grep -iF 'location: https://jellyfin.primary.test/'")
              client.succeed("curl --cacert ${certificates}/edge.crt -sI --resolve backup.test:443:10.0.0.12 https://backup.test/ | grep -iF 'location: https://jellyfin.backup.test/'")
              client.succeed("test $(curl --cacert ${certificates}/edge.crt -sS -o /dev/null -w '%{http_code}' -H 'Host: unknown.primary.test' --resolve jellyfin.primary.test:443:10.0.0.11 https://jellyfin.primary.test/) = 404")
              client.succeed(
              ...
              for certificate in ("untrusted", "wrong-name", "origin"):
                  ...
                  for hostname, address in (("jellyfin.primary.test", "10.0.0.11"), ("jellyfin.backup.test", "10.0.0.12")):
                      client.succeed(
