...
{
  # Full replacement acceptance is x86_64/KVM-only. Other systems keep the
  # static and package checks without exposing a non-authoritative VM fixture.
  perSystem =
    { pkgs, system, ... }:
    lib.optionalAttrs (system == "x86_64-linux") (
      let
        ...
        '';
        # Rewrite only the root Application and its AppProject source allowlist
        # for the disposable Git origin. Bootstrap still uses the shipped seed.
        testSeed = pkgs.runCommand "prod-home-test-seed" { nativeBuildInputs = [ pkgs.yq-go ]; } ''
          mkdir -p "$out"
          cp ${seedManifests}/* "$out/"
          chmod -R u+w "$out"
          yq -i '
            (select(.kind == "Application") | .metadata.name) = "recovery-test-apps"
            | (select(.kind == "Application") | .spec.source.repoURL) = "git://${bridgeAddress}/recovery.git"
            | (select(.kind == "Application") | .spec.source.path) = "./apps"
            | (select(.kind == "AppProject") | .spec.sourceRepos) = ["git://${bridgeAddress}/recovery.git"]
          ' "$out/root.yaml"
        '';
          ...
          (pkgs.testers.runNixOSTest {
            ...
            nodes.fixture-host =
              { pkgs, ... }:
              {
                ...
                # Bootstrap restages through this unit, not the direct script.
            ...
            testScript = ''
              start_all()
              # This is a production-host prerequisite, not a fixture repair:
              # Incus must consume the evaluated host's shared root mount.
              fixture_host.succeed(
                  "findmnt -n -o PROPAGATION / | grep -Eq "
                  "'(^|,)shared(:[0-9]+)?(,|$)'"
              )
