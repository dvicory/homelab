...
def cleanup_variant(
    ...
    for label, argv, timeout in commands:
        result = command(
            directory, label, argv, timeout=timeout, check=False,
            input_text="yes\n" if label == "cleanup-project" else "",
        )
...
def run_variant(
    ...
    try:
        ...
        command(directory, "namespace-create", kube_command(incus, project, "create", "namespace", "jellyfin"))
        for resource in (
