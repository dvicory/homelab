  ...
  den.aspects.disk.luks-storage = {
    ...
    nixos =
      ...
      let
        ...
        formatter = pkgs.callPackage (self + "/pkgs/by-name/prepare-luks-storage/package.nix") { };
        formatterWrappers = lib.mapAttrsToList (
          name: d:
          let
            descriptor = pkgs.writeText "luks-storage-${name}-descriptor" ''
              VERSION=1
              DISK_NAME=${name}
              DECLARED_DEVICE=${d.device}
              MAPPER=${d.mapperName}
              KEY_FILE=/run/agenix/luks-${name}-key
              MOUNTPOINT=${d.mountpoint}
              FS_TYPE=${d.fsType}
            '';
          in
          pkgs.writeShellApplication {
            name = "prepare-luks-storage-${name}";
            runtimeInputs = [ formatter ];
            text = ''
              exec ${formatter}/bin/prepare-luks-storage "$@" --descriptor ${lib.escapeShellArg descriptor}
            '';
          }
        ) cfg;
      in
      lib.mkIf (cfg != { }) {
        environment.systemPackages = formatterWrappers;
