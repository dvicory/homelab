{
  den.aspects.kubernetes.services.monitoring.compute-resources = {
    retainedPaths = {
  ...
  den.aspects.kubernetes.services.monitoring.k8s-manifests =
    ...
    let
      ...
      nodeSelector = {
        "kubernetes.io/hostname" = compute.instance;
      };
      resources = memory: {
        requests = {
          cpu = "50m";
      ...
      securityContext = name: {
        runAsNonRoot = true;
        runAsUser = compute.retainedPaths."monitoring-${name}".uid;
        runAsGroup = compute.retainedPaths."monitoring-${name}".gid;
      ...
      disks = {
        prometheus = {
          size = "20Gi";
          claim = "prometheus-data-prometheus-monitoring-prometheus-0";
        };
        alertmanager = {
          size = "1Gi";
          claim = "alertmanager-data-alertmanager-monitoring-alertmanager-0";
        };
        grafana = {
          size = "2Gi";
          claim = "grafana-data";
    ...
    assert lib.assertMsg (
      ...
      && route.service == "monitoring-grafana"
      && route.port == 80
      && !route.backendTLS
    ) "Grafana route must target its declared HTTP Service monitoring/monitoring-grafana:80";
    {
      applications = {
        monitoring-retained = {
          namespace = "monitoring";
            ...
            lib.mapAttrsToList (name: disk: [
              {
                ...
                spec = {
                  ...
                  storageClassName = "";
                  local.path = compute.retainedPaths."monitoring-${name}".guestPath;
                  claimRef = {
                    namespace = "monitoring";
                    name = disk.claim;
                  };
          ...
          helm.releases.monitoring = {
            ...
            values = {
              ...
              prometheus.prometheusSpec = {
                ...
                securityContext = securityContext "prometheus";
              ...
              alertmanager = {
                ...
                alertmanagerSpec = {
                  ...
                  inherit nodeSelector;
                  securityContext = securityContext "alertmanager";
                  # Use the host-owned volume root, not a kubelet-created root-owned subdirectory.
              ...
              grafana = {
                ...
                inherit nodeSelector;
                securityContext = securityContext "grafana";
              ...
              additionalPrometheusRulesMap.household.groups = [
                {
                  ...
                  rules = [
                    (alert "ServiceUnavailable"
                      ''kube_deployment_status_replicas_available{namespace=~"jellyfin|immich|media|identity|monitoring"} < kube_deployment_spec_replicas{namespace=~"jellyfin|immich|media|identity|monitoring"}''
                      "5m"
                      "Deployment unavailable: {{ $labels.namespace }}/{{ $labels.deployment }}"
                      "Inspect deployment conditions, pod probes, events, runtime Secrets and retained volume bindings."
                    )
                    (alert "StatefulServiceUnavailable"
                      ''kube_statefulset_status_replicas_ready{namespace=~"immich|monitoring|identity"} < kube_statefulset_replicas{namespace=~"immich|monitoring|identity"}''
                      "5m"
                      "Stateful service unavailable: {{ $labels.namespace }}/{{ $labels.statefulset }}"
                      "Inspect readiness, storage mounts and runtime credentials before restarting a stateful writer."
                    )
                    (alert "MetricsTargetUnavailable" "up == 0" "5m"
                      "Metrics target unavailable: {{ $labels.job }} / {{ $labels.instance }}"
                      "Inspect the exporter, private network path and Prometheus target error; missing metrics do not indicate health."
                    )
                    (alert "MetricsCollectionMissing" "absent_over_time(up[15m])" "5m"
                      "All metrics collection targets are missing"
                      "Inspect Prometheus target discovery and the private scrape path; an absent target set is not healthy."
                    )
                    (alert "ClusterInventoryMissing" "absent_over_time(kube_node_info[15m])" "5m"
                      "Kubernetes inventory metrics are missing"
                      "Check kube-state-metrics and its ServiceMonitor before trusting workload availability panels."
                    )
                    (alert "PersistentStoragePressure"
                      "kubelet_volume_stats_available_bytes / kubelet_volume_stats_capacity_bytes < 0.15"
                      "10m"
                      "Persistent volume low on space: {{ $labels.namespace }}/{{ $labels.persistentvolumeclaim }}"
                      "Inspect the owning workload and retained filesystem. Reduce retention or increase capacity; do not remove household data to make room for monitoring."
                    )
                    (alert "NodeStoragePressure"
                      ''node_filesystem_avail_bytes{fstype!~"tmpfs|ramfs|overlay|squashfs"} / node_filesystem_size_bytes < 0.15''
                      "10m"
          ...
          helm.releases.loki = {
            ...
            values = {
              ...
              loki = {
                ...
                podSecurityContext = securityContext "loki";
